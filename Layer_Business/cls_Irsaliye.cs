...
namespace Layer_Business
{
    public class cls_Irsaliye :INotifyPropertyChanged
    {
        ...
        public ObservableCollection<cls_Irsaliye> populateIrsaliyeView(string irsaliyeNo) 
        {
            try
            { 
            dataTable = dataLayer.Select_Stored_Proc_LTD("vbpIrsaliyeDuzenle","@yil","@irsaliyeNo", Variables.Yil_, irsaliyeNo,Variables.Yil_);
        ...
        {
            Variables.Query_ = $"select top 1 * from vbvIrsaliyeGiris where CARI_KOD='{CariKodu}'";
            dataTable = dataLayer.Select_CommandLtd(Variables.Query_, Variables.Yil_);
