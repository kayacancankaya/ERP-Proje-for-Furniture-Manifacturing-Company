...
{ 
    public class LoginLogic : INotifyPropertyChanged
    {
        ...
        public bool CheckIfUserExists(string username)
        {
            ...
            string query = $"select count(*) from vbtUserInfo where kullaniciAdi='{username}'";

            reader = dataLayer.Select_Command_Data_Reader(query, Variables.CurrentYear_);
			while(reader.Read())
            {
        ...
        private bool CheckIfPasswordExists(string hashedPassword)
        {
            ...
            string query = $"select count(*) from vbtUserInfo where sifre='{hashedPassword}'";
		...
		public bool CheckIfPcAddressExists()
        {
            ...
            string query = $"select count(*) from vbtUserInfo where adres='{hashedAddr}'";
        ...
        public void UpdatePCAutoLoginStatustoZero()
        {
            ...
            string query = $"update vbtUserInfo set OtoLogin='0' where adres='{hashedAddr}'";

            dataLayer.Update_Statement(query, Variables.CurrentYear_);
            }
        ...
        public bool SaveNewUser(string user, string password, string department, string email)
        {
            ...
            try
            {
                string query = string.Format("insert into vbtUserInfo (KullaniciAdi,sifre,Departman,Email,Adres,OtoLogin) values('{0}','{1}','{2}','{3}','{4}',1)", user, hashedPass, department, email, hashedAddr);
                dataLayer.Insert_Statement(query, Variables.CurrentYear_, "Kullanıcı", 1);
            }
        ...
        public bool SavePaswordChanged(string user, string password)
        {
            ...
            try
            {
                string query = string.Format("update vbtUserInfo set sifre='{0}' where Adres='{1}' and KullaniciAdi='{2}'", hashedPass, hashedAddr, user);
                dataLayer.Update_Statement(query, Variables.CurrentYear_, "Kullanıcı", 1);
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); return false; }
        ...
        public int CheckLoginAttemp(string username,string passwordentry)
        {
            ...
            string query = $"select count(*) from vbtUserInfo where sifre='{hashedPass}' and kullaniciAdi='{username}'";
        ...
        public int IfStatusAutoLogin()
        {
            ...
            string query = $"select top 1 OtoLogin from vbtUserInfo where adres='{hashedAddr}'";
        ...
        public string GetUserName()
        {
            ...
            try
            {
                string query = $"select top 1 KullaniciAdi from vbtUserInfo where Adres='{hashedAddr}'";
        ...
        public async Task<string> GetUserNameAsync()
        {
            ...
            try
            {
				...
				reader = await dataLayer.Select_Command_Data_ReaderAsync(query, Variables.CurrentYear_);
        ...
        public async Task<string> GetEmailAddressAsync()
        {
            ...
            try
            {
                string query = $"select top 1 Email from vbtUserInfo where Adres='{hashedAddr}'";
        ...
        public string GetUserName(string HashedAddress) 
        {
            try
            { 
                string query = $"select top 1 KullaniciAdi from vbtUserInfo where Adres='{HashedAddress}'";
        ...
        public string GetFabrika()
        {
            try
            {
                ...
                string hashedAddress = GetAddress();

                string query = string.Format("Select top 1 departman from vbtUserInfo where Adres='{0}'", hashedAddress);
                ObservableCollection<LoginLogic> login = dataLayer.Select_Command_Data(query, Variables.Yil_,"Vita", reader =>
                {
                    LoginLogic model = new();
                    model.Departman = reader["departman"] is DBNull ? "" : reader["departman"].ToString();
                    return model;
                });

                Departman = login.Select(d=>d.Departman).FirstOrDefault();
        ...
        public async Task<string> GetFabrikaAsync()
        {
            try
            { 
                string fabrika = string.Empty;
                ...
                string query = string.Format("Select top 1 departman from vbtUserInfo where Adres='{0}'", hashedAddress);
                ObservableCollection<LoginLogic> login = await dataLayer.Select_Command_Data_Async(query,Variables.Yil_,"Vita", reader =>
                {
                    LoginLogic model = new();
                    model.Departman = reader["departman"] is DBNull ? "Vita" : reader["departman"].ToString();
                    return model;
                });
		...
		public string GetDepartment()
		{
			...
			try
			{
				string query_ = $"select top 1 Departman from vbtUserInfo where Adres='{hashedAddr}'";

				reader = dataLayer.Select_Command_Data_Reader(query_, Variables.CurrentYear_);
		...
		public async Task<string> GetDepartmentAsync()
		{
			...
			try
			{
				string query = $"select top 1 Departman from vbtUserInfo where Adres='{hashedAddr}'";
        ...
        public string GetUserNameFromUserID(int userID)
        {
            ...
            try
            {
                string query = $"select top 1 KullaniciAdi from vbtUserInfo where UserID='{userID}'";
        ...
        public void UpdateAutoLoginStatus(string user, string password) 
        {
            try
            {
                ...
                {
                    query = $"update vbtUserInfo set OtoLogin=0 where KullaniciAdi='{user}' and sifre='{hashedPass}'";
                }
                else
                {
                    query = $"update vbtUserInfo set OtoLogin=1 where KullaniciAdi='{user}' and sifre='{hashedPass}'";
                }
