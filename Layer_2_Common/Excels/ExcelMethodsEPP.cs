...
namespace Layer_2_Common.Excels
{
    public class ExcelMethodsEPP
    {
        ...
        public DataTable ReadExcelFile(string filePath, string worksheetName,int startingColumn,int endingColumn
                                        ,int startingRow)
        {
            try
            {
        ...
        public static void TextConditionCellValue(ExcelPackage existingPackage, string worksheetName, string range,string cellValue,string newValue,
                                                    string? cellValue2, string? newValue2)
        {
            try
            {
                var worksheet = existingPackage.Workbook.Worksheets[worksheetName];
                if (cellValue == worksheet.Cells[range].Value)
                {
                    worksheet.Cells[range].Value = newValue;
                    existingPackage.Save();

                }
                else if (!string.IsNullOrEmpty(cellValue2))
                {
                    if (cellValue2 == worksheet.Cells[range].Value)
                    {
                        worksheet.Cells[range].Value = newValue;
                        existingPackage.Save();
                    }
        ...
        public static void ColorConditionCellValue(ExcelPackage existingPackage, string worksheetName, string range,string cellValue,string colorCode,
                                                    string? cellValue2,string? colorCode2)
        {
            try
            {
                var worksheet = existingPackage.Workbook.Worksheets[worksheetName];
                ...
                { 
                    if(cellValue2 == worksheet.Cells[range].Value)
                    {
                        var cellStyle = worksheet.Cells[range].Style;
                        cellStyle.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                        cellStyle.Fill.BackgroundColor.SetColor(ColorTranslator.FromHtml(colorCode2));
                        existingPackage.Save();
                    }
                }
        ...
        public void InsertPdf(ExcelPackage existingPackage, string worksheetName, string pdfPath, int columnNo, int rowNo)
        {
            var worksheet = existingPackage.Workbook.Worksheets[worksheetName];

            var pdfBytes = File.ReadAllBytes(pdfPath);
            var base64Pdf = Convert.ToBase64String(pdfBytes);
            worksheet.Cells[rowNo, columnNo].Value = base64Pdf;
        ...
        public void ExportDataToExcelMergedCellExists(DataTable dataTable, ExcelPackage existingPackage, string worksheetName, int rowNumber, int columnNumber,int mergedCellRow,int offsetForMerged)
        {
            ...
            {
                if(j >= mergedCellRow)
                    worksheet.Cells[rowNumber, columnNumber + j + offsetForMerged].Value = dataTable.Columns[j].ColumnName;
                else
                    worksheet.Cells[rowNumber, columnNumber + j].Value = dataTable.Columns[j].ColumnName;
            }
            rowNumber++;
            ...
            for (int i = 0; i < dataTable.Rows.Count; i++)
            {
                ...
                {
                    if (j >= mergedCellRow)
                        worksheet.Cells[rowNumber + i, columnNumber + j + offsetForMerged].Value = dataTable.Rows[i][j];
                    else
                        worksheet.Cells[rowNumber + i, columnNumber + j].Value = dataTable.Rows[i][j];
                }
            }
        ...
        public void ChangeNumberFormat(ExcelPackage existingPackage, string sheetName, string range, string numberFormat)
        {
            var worksheet = existingPackage.Workbook.Worksheets[sheetName];
        ...
        public string GetFirstBlankCell(ExcelPackage existingPackage, string sheetName, string startingRange)
        {
            ExcelWorksheet worksheet = existingPackage.Workbook.Worksheets[sheetName];

            if (worksheet.Cells[startingRange].Value == null || worksheet.Cells[startingRange].Value.ToString() == "")
                return startingRange;

            int startColumn = worksheet.Cells[startingRange].Start.Column;
            int row = worksheet.Cells[startingRange].Start.Row;
            int currentRow = row;

            do
            {
                currentRow++; // Move to the next column horizontally
                string nextCell = worksheet.Cells[currentRow, startColumn].Address;

                if (worksheet.Cells[currentRow, startColumn].Value == null || worksheet.Cells[currentRow, startColumn].Value.ToString() == "")
                    return nextCell;
