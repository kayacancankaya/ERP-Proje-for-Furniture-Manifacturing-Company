...
{
    /// <summary>
    /// Interaction logic for Frm_Recete_Guncelle.xaml
    /// </summary>
    public partial class Frm_Recete_Guncelle : Window
    {
        Variables variables = new();
        Cls_Isemri isemri = new();

        ObservableCollection<Cls_Isemri> receteCollection = new();
        ObservableCollection<Cls_Isemri> guncellemeCollection = new();
        Dictionary<string, string> restrictionPairs = new Dictionary<string, string>();
        ObservableCollection<Cls_Isemri> mamulDetayCollection = new();
        ObservableCollection<Cls_Isemri> receteEkleCollection = new();
        bool tamamlananlariListeleme = false;
        public Frm_Recete_Guncelle()
        {
        ...
        private void ChangeVisibility(string comboBoxItemTagName)
        {
            switch (comboBoxItemTagName)
            {
                case "hamkodu":
                    ...
                    dg_ReceteEkleSecim.Visibility = Visibility.Collapsed;
                    break;
                case "miktar":
        ...
        private void txt_eklenecek_mamul_kodu_changed(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    CRUDmessages.GeneralFailureMessage("Mamul Kodu 11 Karakterden Küçük Olamaz.");
                    return;
                }
                if (txt_eklenecek_mamul_kodu.Text.Length > 35)
                {
                    CRUDmessages.GeneralFailureMessage("Mamul Kodu 35 Karakterden Büyük Olamaz.");
                    return;
                }

                string opno = isemri.GetLastOpNo(txt_eklenecek_mamul_kodu.Text);
        ...
        private void btn_mamul_detay_clicked(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                bool tamamlananlariListeleme = false;
                if (cb_tamamlananlari_listeleme_ekle.IsChecked == true)
                    tamamlananlariListeleme = true;

                mamulDetayCollection = isemri.PopulateMamulReceteList(txt_eklenecek_mamul_kodu.Text, tamamlananlariListeleme);
        ...
        private void btn_listele_clicked(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(txt_takip_no.Text))
                {
                    ...
                    {
                        CRUDmessages.GeneralFailureMessageCustomMessage("Referans İşemrine 3 Karakterden Az Giriş Yapılamaz.");
                        return;
                    }
                ...
                if (!string.IsNullOrWhiteSpace(txt_isemrino.Text))
                {
                    ...
                    {
                        CRUDmessages.GeneralFailureMessageCustomMessage("İşemrinoya 3 Karakterden Az Giriş Yapılamaz.");
                        return;
                    }
                ...
                if (cb_tamamlananlari_listeleme.IsChecked == true)
                    restrictionPairs.Add("@tamamlananlariListeleme", "true");

                Mouse.OverrideCursor = Cursors.Wait;

                receteCollection = isemri.PopulateReceteGuncelleList(restrictionPairs);
        ...
        private async void btn_recete_guncelle_clicked(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (comboBoxItemTag == "ekle")
                {
                    ...
                    if (txt_eklenecek_ham_kodu.Text.Length < 11)
                    {
                        CRUDmessages.GeneralFailureMessageCustomMessage("Ham Kodu 11 Karakterden Küçük Olamaz");
                        return;
                    }
                    if (txt_eklenecek_ham_kodu.Text.Length > 35)
                    {
                        CRUDmessages.GeneralFailureMessageCustomMessage("Ham Kodu 35 Karakterden Büyük Olamaz");
                        return;
                    }
                    ...
                    if (txt_eklenecek_mamul_kodu.Text.Length < 11)
                    {
                        CRUDmessages.GeneralFailureMessageCustomMessage("Mamul Kodu 11 Karakterden Küçük Olamaz");
                        return;
                    }
                    if (txt_eklenecek_mamul_kodu.Text.Length > 35)
                    {
                    ...
                    Variables.Result_ = await isemri.CheckIfOpnoExists(txt_opno.Text, txt_eklenecek_mamul_kodu.Text);
                ...
                if (comboBoxItemTag != "ekle")
                {
                    ...
                    foreach (Cls_Isemri ie in dg_ReceteSecim.Items)
                    {
                        if (ie.IsChecked == true)
                        {
                            ...
                            if (comboBoxItemTag == "hamkodu" ||
                                comboBoxItemTag == "hammiktar")
                            {
                                ie.EskiHamKodu = ie.HAM_KODU;
                                ie.HAM_KODU = txt_yeni_ham_kodu.Text;
                            }
                            if (comboBoxItemTag == "miktar" ||
                                comboBoxItemTag == "hammiktar")
                            {
                                ie.EskiReceteMiktar = ie.RECETE_MIKTAR;
                                ie.RECETE_MIKTAR = Convert.ToDecimal(txt_miktar.Text, CultureInfo.InvariantCulture);
                            }

                            guncellemeCollection.Add(ie);
                            Variables.Counter_++;
                        }
                    }
                ...
                if (comboBoxItemTag == "ekle")
                {
                    foreach (Cls_Isemri eklenecek in dg_ReceteEkleSecim.Items)
                    {
                        if (eklenecek.IsChecked == true)
                        {
                            eklenecek.EskiReceteMiktar = 0;
                            eklenecek.RECETE_MIKTAR = Convert.ToDecimal(txt_eklenecek_miktar.Text, CultureInfo.InvariantCulture);
                            eklenecek.EskiHamKodu = string.Empty;
                            eklenecek.HAM_KODU = txt_eklenecek_ham_kodu.Text;
                            eklenecek.MAMUL_KODU = txt_eklenecek_mamul_kodu.Text;
                ...
                Variables.ResultInt_ = await isemri.UpdateUrunAgaciAsync(guncellemeCollection, comboBoxItemTag);
        ...
        private void btn_isemri_liste_clicked(object sender, EventArgs e)
        {
            try
            {
                dg_ReceteSecim.Visibility = Visibility.Collapsed;
                ...
                receteEkleCollection = isemri.PopulateReceteEkleList(txt_eklenecek_mamul_kodu.Text, tamamlananlariListeleme);

                if (receteEkleCollection == null)
                {
                    CRUDmessages.GeneralFailureMessage("Mamulün Reçetesi Listelenirken"); Mouse.OverrideCursor = null; return;
                }

                if (!receteEkleCollection.Any())
                {
                    CRUDmessages.QueryIsEmpty(); Mouse.OverrideCursor = null;
                }

                dg_ReceteEkleSecim.ItemsSource = receteEkleCollection;
                dg_ReceteEkleSecim.Visibility = Visibility.Visible;
                Mouse.OverrideCursor = null;
            }
            catch
            {
        ...
        private void ToggleHeaderCheckBox(object sender, RoutedEventArgs e)
        {
            ...
            foreach (Cls_Isemri item in dg_ReceteSecim.Items)
            {
                item.IsChecked = headerIsChecked;
            }
        }
        private void ToggleHeaderCheckBoxEkle(object sender, RoutedEventArgs e)
        {
            bool headerIsChecked = ((CheckBox)sender).IsChecked ?? false;
            foreach (Cls_Isemri item in dg_ReceteEkleSecim.Items)
            {
                item.IsChecked = headerIsChecked;
            }
        }
