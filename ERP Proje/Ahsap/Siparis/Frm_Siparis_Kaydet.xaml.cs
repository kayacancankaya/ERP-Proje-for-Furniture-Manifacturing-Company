    ...
    public partial class Frm_Siparis_Kaydet : Window
    {
            ...
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    Cls_Siparis siparis = new Cls_Siparis
                    {
                        ...
                        SiparisFiyat = Convert.ToSingle(row["Fiyat"]),
        ...
        private void btn_siparis_kaydet_clicked(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    variables.QumulativeSum += Convert.ToSingle(item.SiparisMiktar) * item.SiparisFiyat;
                    kumulatifKDV += (Convert.ToSingle(item.SiparisMiktar) * Convert.ToSingle(item.SiparisFiyat)) / 100 * Convert.ToSingle(item.StokKDV);
                    variables.Counter++;
                }
